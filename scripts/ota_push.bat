@echo off
:: Windows counterpart of scripts/ota_push.sh — POST a built image to the head
:: unit's /ota endpoint over Wi-Fi.
::
:: Usage:
::   scripts\ota_push.bat                     192.168.4.1 (SoftAP gateway)
::   scripts\ota_push.bat android-auto.local
::   set BIN=build_waveshare\esp32p4_android_auto.bin ^& scripts\ota_push.bat
...
if not exist "%BIN%" (
    echo ota_push: %BIN% not found - run idf.py build first 1>&2
    exit /b 1
)
...
if %errorlevel% neq 0 (
    echo ota_push: upload failed 1>&2
    exit /b 1
)
echo.
