...
where adb >nul 2>nul
if %errorlevel% neq 0 (
    echo install_app: adb not found - install Android platform-tools ^>&2
    exit /b 1
)
...
if "%ARG%"=="" (
    set APK=
    for /f "delims=" %%I in ('python -c "import glob, os; a=sorted(glob.glob('%REL:\=\\%/aa_bridge-*.apk'), key=os.path.getmtime); print(a[-1] if a else '')"') do set APK=%%I
    if "!APK!"=="" (
        echo install_app: no release\aa_bridge-*.apk found - run scripts\release.bat first ^>&2
        exit /b 1
    )
) else (
    if exist "%ARG%" (
...
if not exist "!APK!" (
    echo install_app: !APK! not found ^>&2
    exit /b 1
)
...
if !N! gtr 1 (
    if not defined ANDROID_SERIAL (
        echo install_app: !N! devices connected - pin one with set ANDROID_SERIAL=^<serial^> ^>&2
        adb devices ^>&2
        exit /b 1
    )
)
