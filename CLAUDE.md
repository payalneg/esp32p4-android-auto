...
## Связь с контроллером: CAN или Bluetooth

Прошивка говорит с VESC по одному из двух транспортов; выбор — на экране
Settings → **VESC link**, применяется сразу (без перезагрузки) и хранится в NVS
(`vesc_cfg/link_ble`).

- **CAN** (по умолчанию) — как раньше, TWAI-шина, `comm_can.c`.
- **BLE** — головное устройство работает GATT-централом и подключается к
  **VESC Express** по Nordic UART Service, ровно как VESC Tool. Провода CAN не
  нужны. Адаптер выбирается сканированием на экране VESC link, адрес лежит в
  отдельном NVS-неймспейсе `vescble` (переживает Reset настроек).

Граница «протокол / транспорт» — `components/vesc_can/vesc_link.{c,h}`:
все потребители (RT-данные, LISP-панель, PAS, загрузчик LISP, конфигуратор,
NUS-мост для телефона) зовут `vesc_link_send[_sync]()`, а приём с обоих
транспортов сходится в единственный `vesc_packet_dispatch()` в `main.c`.

Что стоит знать про BLE-ветку:

- **`COMM_FORWARD_CAN`.** VESC Express сам сидит на CAN рядом с контроллером,
  поэтому payload заворачивается в `[34][Target VESC ID]` — это и есть «scan
  CAN bus» из VESC Tool. Выключается свитчем «Controller is behind the
  adapter», если адаптер сам является целью.
- **Два таска** (`main/vesc_ble_link.c`): `vesc_ble_tx` держит «один запрос в
  полёте» и ждёт ответ, `vesc_ble_rx` разбирает пакеты. Ждать ответа на
  NimBLE host task нельзя — ответ приходит на него же, будет дедлок; поэтому
  `vesc_link_send()` только кладёт в очередь.
- **Корреляция ответов** — только по порядку прихода, поэтому асинхронный
  `COMM_LISP_PRINT` / `COMM_PRINT` намеренно не засчитывается как ответ.
- **Тайминги** (`components/vesc_can/Kconfig`): `VESC_BLE_SYNC_TIMEOUT_MS`
  (300), `VESC_BLE_INTERVAL_PCT` (200 — все периодические опросы вдвое реже),
  `VESC_BLE_CONN_ITVL_MS` (15). Один round-trip = два интервала соединения.
- **LISP-скрипт.** В запросах панели поле `reply_can_id` при BLE равно **255**
  (широковещательный CAN-адрес, занять его некому), и скрипт отвечает
  `(send-data pbuf)` — на тот интерфейс, откуда пришёл запрос. Правка есть в
  `lisp/main.lisp` и `lisp/main.ru.lisp`; со старым скриптом на контроллере по
  BLE не будет выдвижной панели и cruise/profile на дашборде.
- **Чего нет по BLE:** головное устройство не видно в CAN-скане VESC Tool (нас
  нет на шине), а температуры второй головы вместо широковещательных `STATUS_4`
  опрашиваются раз в секунду (`vesc_head2_poll_loop`).
- **Бюджет BLE:** `CONFIG_BT_NIMBLE_MAX_CONNECTIONS=5` (2 периферийных —
  телефон и VESC Tool, 3 центральных — каденс, скорость, адаптер) и
  `ARB_MAX_SLOTS 3` в `main/ble_central_arb.c`. VESC Express держит только
  одно соединение: если к нему подключён VESC Tool с телефона, дисплей не
  подключится.
