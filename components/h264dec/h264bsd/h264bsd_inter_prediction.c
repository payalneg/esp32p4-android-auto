...
/* Code */
    ...
    switch (pMb->mbType)
    {
        ...
        case P_L0_16x16:
            ...
            /* A skipped macroblock with a zero motion vector IS the
             * co-located macroblock of the reference. Upstream still runs it
             * through the interpolator into the 384-byte MB buffer and then
             * h264bsdWriteMacroblock copies that into the picture — two
             * passes over PSRAM for every one of the 1500 MBs of a static
             * 800x480 frame, ~28 ms. Copy it once, straight across. */
            if (pMb->mbType == P_Skip &&
                pMb->mv[0].hor == 0 && pMb->mv[0].ver == 0)
            {
                if (pMb->decoded <= 1 &&
                ...
                {
                    if (g_h264bsd_skip_ref) g_h264bsd_skip_ref_mixed = 1;
                    g_h264bsd_skip_ref = refImage.data;
                }
                return(HANTRO_OK);
            }
#endif
...
void h264bsdCopyMbFromRef(image_t *currImage, const image_t *refImage,
                          u32 colPx, u32 rowPx)
{
    u32 width = currImage->width * 16;          /* luma stride in bytes */
    u32 cw    = width >> 1;                     /* chroma stride */
    u32 lumaSize = width * currImage->height * 16;

    const u64 *src = (const u64*)(refImage->data + rowPx * width + colPx);
    u64 *dst       = (u64*)currImage->luma;
    for (u32 i = 16; i; i--)
    {
        dst[0] = src[0]; dst[1] = src[1];
        src = (const u64*)((const u8*)src + width);
        dst = (u64*)((u8*)dst + width);
    }

    const u8 *cbSrc = refImage->data + lumaSize + (rowPx >> 1) * cw + (colPx >> 1);
    const u8 *crSrc = cbSrc + (lumaSize >> 2);
    u8 *cbDst = currImage->cb;
    u8 *crDst = currImage->cr;
    for (u32 i = 8; i; i--)
    {
        *(u64*)cbDst = *(const u64*)cbSrc;
        *(u64*)crDst = *(const u64*)crSrc;
        cbSrc += cw; crSrc += cw; cbDst += cw; crDst += cw;
    }
}
#endif
