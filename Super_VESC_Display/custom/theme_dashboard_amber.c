...
static void amber_screen_init(lv_ui *ui)
{
    ...
    /* Apply the saved brightness-gesture setting.  The generated code starts
     * the Amber brightness slider hidden; when the user has the gesture
     * enabled we need to un-hide it so swipe-to-brightness works. */
    if (ui->dashboard_Amber_brightness_slider) {
        if (settings_wrapper_get_brightness_gesture_enabled())
            lv_obj_clear_flag(ui->dashboard_Amber_brightness_slider, LV_OBJ_FLAG_HIDDEN);
        else
