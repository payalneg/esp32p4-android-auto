...
```text
 Батарея 12 В        ┌────────────┐       ┌──────────────────┐        ┌────────────┐
 (шина VESC) ───────▶│   DC-DC    │──────▶│  Плата ESP32-P4  │───────▶│ WROOM 3V3  │
              +12 В  │ 12 В → 5 В │ +5 В  │  USB-C / 5V pin  │  3V3   │  (опция)   │
                     │   ≥ 1 А    │       │ (встроенный LDO) │  (J3)  └────────────┘
                     └────────────┘       └──────────────────┘
```
...
```text
  ESP32-P4                        TJA1051 (лучше T/3)         CAN-шина
 ┌─────────────────┐            ┌───────────────┐
 │                 │            │               │
 │ GPIO 48 (TX) ───┼───────────▶│ TXD           │           ┌─────────────┐
 │                 │            │               │           │             │
...
- **Номиналы делителя на RXD**: 1.8 кОм (последовательно) + 3.3 кОм (на землю)
  или 10 кОм + 18 кОм. TJA1051 питается от 5 В, поэтому его RXD выдаёт 5 В —
  делитель опускает уровень до безопасных для P4 3.3 В. На TXD ничего не нужно:
  3.3 В с ноги P4 трансивер понимает напрямую.
- **Терминатор 120 Ом** между CANH ↔ CANL — **обязателен**: CAN-шина без
  терминации работает через раз или не работает вовсе. На большинстве готовых
  модулей TJA1051 он уже распаян (ищите резистор с маркировкой `121` между
  клеммами CANH и CANL или померьте CANH↔CANL на обесточенном модуле:
  ~120 Ом — есть, обрыв — нет). Если на вашем модуле его нет — **допаяйте
  резистор 120 Ом между CANH и CANL**, как показано на схеме.
...
```text
  ESP32-P4 (J3 хедер)              ESP32-WROOM-32 / D1 Mini
 ┌─────────────────────┐          ┌─────────────────────────┐
 │ GPIO 22 (UART1 TX) ─┼─────────▶│ GPIO 3 (RX0)            │
 │ GPIO 21 (UART1 RX) ◀┼──────────┤ GPIO 1 (TX0)            │
 │ GPIO 24 ────────────┼─────────▶│ EN     — reset чипа     │
 │ GPIO 25 ────────────┼─────────▶│ GPIO 0 — выбор boot     │
 │ 3V3 (выход) ────────┼─────────▶│ 3V3    — питание        │
 │ GND ────────────────┼──────────┤ GND                     │
 └─────────────────────┘          └─────────────────────────┘
```
...
- **Из браузера**: `http://android-auto.local/files` (или IP шлюза SoftAP) —
  тот же обзор / превью / загрузка / переименование / перемещение / удаление
  по обоим дискам.

### Веб-редактор LISP

`http://android-auto.local/lisp` — полноценный редактор LispBM-скрипта,
который крутится на VESC, из любого браузера в той же сети (телефон и так
подключается к SoftAP головного устройства):

- подсветка синтаксиса, номера строк, радужные и парные скобки, поиск/замена,
  блочный отступ, автозакрытие;
- линтер на те ошибки, которые здесь дороже всего обходятся: баланс скобок,
  незакрытые строки, парность `@const-start` / `@const-end`, defun'ы,
  оставшиеся вне const-блока, и потоки, запущенные раньше, чем определена
  функция, которую они выполняют;
- библиотека скриптов на устройстве (`/vescfs/lisp` и microSD): открыть,
  сохранить, переименовать / переместить, удалить, создать папку, залить файл;
- **Read VESC** / **Upload** / **Upload + Run** / **Start** / **Stop** по CAN
  с прогресс-баром (передача идёт асинхронно, остальной сервер не блокируется);
- живая консоль вывода `(print ...)` и строка REPL — выполнить выражение без
  перезаливки скрипта;
- статистика LISP (CPU / heap / память / стек и экспортированные переменные),
  опрашивается только пока открыта эта вкладка.

Выключается через `CONFIG_LISP_HTTP_ENABLED=n` (стоит ~15 КБ флеша).
`scripts/lisp_web_mock.py` отдаёт ту же страницу поверх фейкового устройства —
чтобы дорабатывать редактор без платы.
